...
const SALARY_CALC_CONFIG = {
    YEAR: 2023,
    NATIONAL_PENSION: { RATE_EMPLOYEE: 0.045, MIN_MONTHLY_INCOME: 370000, MAX_MONTHLY_INCOME: 5900000, },
    EMPLOYMENT_INSURANCE: { RATE_EMPLOYEE_UNEMPLOYMENT_BENEFIT: 0.009, },
    HEALTH_INSURANCE: { RATE_EMPLOYEE: 0.03545, MIN_MONTHLY_PREMIUM_EMPLOYEE: 9890, MAX_MONTHLY_PREMIUM_EMPLOYEE: 3911280, },
    LONG_TERM_CARE_INSURANCE: { RATE_OF_HEALTH_INSURANCE: 0.1281, },
    INCOME_TAX: {
        DEDUCTION_BRACKETS: [
            { limit: 5000000, rate: 0.70, base: 0, over: 0 },
        ...
        TAX_CREDIT: {
            STANDARD_AMOUNT: 1300000, RATE1: 0.55, RATE2: 0.30, BASE_CREDIT_FOR_RATE2: 715000,
            LIMITS: [
...
function createSalaryCalculatorLogic(config) {
    // All calculation functions (calculateNationalPension, ..., calculateNetMonthlyPay) remain unchanged internally
    // These functions are now part of the returned object.
    function calculateNationalPension(annualSalary) {
        const npConfig = config.NATIONAL_PENSION; const monthlySalary = annualSalary / 12;
    ...
    function calculateEmploymentInsurance(annualSalary) {
        const eiConfig = config.EMPLOYMENT_INSURANCE; const monthlySalary = annualSalary / 12;
    ...
    function calculateHealthInsurance(annualSalary) {
        const hiConfig = config.HEALTH_INSURANCE; const monthlySalary = annualSalary / 12;
    ...
    function calculateLongTermCareInsurance(annualHealthInsurancePremium) {
        const ltcConfig = config.LONG_TERM_CARE_INSURANCE; const monthlyHealthInsurancePremium = annualHealthInsurancePremium / 12;
    ...
    function getIncomeDeduction(annualGrossSalary) {
        if (annualGrossSalary <= 0) return 0; const brackets = config.INCOME_TAX.DEDUCTION_BRACKETS;
        let deduction = 0;
        for (const bracket of brackets) {
            if (annualGrossSalary <= bracket.limit) { deduction = bracket.base + (annualGrossSalary - bracket.over) * bracket.rate; break; }
        }
    ...
    function calculateTaxableIncome(annualSalary, nonTaxableAnnualAmount = 0, annualNationalPension, annualHealthInsurance, annualEmploymentInsurance, annualIncomeDeduction, annualHumanDeduction) {
        ...
        let taxable = annualSalary - totalDeductions; return taxable > 0 ? taxable : 0;
    }
    function calculateIncomeTax(taxableIncome) {
        if (taxableIncome <= 0) return 0; const brackets = config.INCOME_TAX.TAX_BRACKETS;
        let tax = 0;
        for (const bracket of brackets) {
            if (taxableIncome <= bracket.limit) { tax = bracket.cumulativeBase + (taxableIncome - bracket.prevLimit) * bracket.rate; break; }
        }
        return Math.floor(tax / 10) * 10;
    }
    function getIncomeTaxCredit(calculatedTax, annualGrossSalary) {
        if (calculatedTax <= 0) return 0; const creditConfig = config.INCOME_TAX.TAX_CREDIT;
        let taxCredit = 0;
        if (calculatedTax <= creditConfig.STANDARD_AMOUNT) taxCredit = calculatedTax * creditConfig.RATE1;
        else taxCredit = creditConfig.BASE_CREDIT_FOR_RATE2 + (calculatedTax - creditConfig.STANDARD_AMOUNT) * creditConfig.RATE2;
        let creditLimit = 0;
        for (const limitInfo of creditConfig.LIMITS) {
            if (annualGrossSalary <= limitInfo.salaryLimit) {
                if (limitInfo.reductionFactor) creditLimit = Math.max(limitInfo.minLimit, limitInfo.creditLimit - (annualGrossSalary - limitInfo.reductionBaseSalary) * limitInfo.reductionFactor);
                else creditLimit = limitInfo.creditLimit;
                break;
            }
        }
    ...
    function calculateFinalIncomeTax(calculatedTax, taxCredit) {
        const finalTax = calculatedTax - taxCredit; return finalTax > 0 ? Math.floor(finalTax / 10) * 10 : 0;
    }
    ...
    function calculateTotalAnnualDeductions(annualSalary, nonTaxableAnnualAmount, dependentsCount) {
        const np = calculateNationalPension(annualSalary); const hi = calculateHealthInsurance(annualSalary);
        const ltc = calculateLongTermCareInsurance(hi); const ei = calculateEmploymentInsurance(annualSalary);
    ...
    return {
        calculateNationalPension, calculateEmploymentInsurance, calculateHealthInsurance, calculateLongTermCareInsurance,
        getIncomeDeduction, getHumanDeduction, calculateTaxableIncome, calculateIncomeTax, getIncomeTaxCredit,
        calculateFinalIncomeTax, calculateLocalIncomeTax, calculateTotalAnnualDeductions, calculateNetMonthlyPay
    };
}
...
function initializeSalaryPage() {
    ...
    function displayResultsUI(annualSalary, nonTaxableMonthly, dependentsCount) {
        try {
            ...
            if (isNaN(annualSalary) || annualSalary <= 0) {
                if (typeof Toast !== 'undefined' && Toast.show) Toast.show('유효한 연봉을 입력해주세요.', 'danger');
                DOMUtils.getElement('#annual_salary').focus(); resultsContainer.style.display = 'none'; return;
            }
            if (isNaN(nonTaxableMonthly) || nonTaxableMonthly < 0) {
                if (typeof Toast !== 'undefined' && Toast.show) Toast.show('유효한 월 비과세액을 입력해주세요.', 'danger');
                DOMUtils.getElement('#non_taxable_monthly').focus(); resultsContainer.style.display = 'none'; return;
            }
            if (isNaN(dependentsCount) || dependentsCount < 1) {
                if (typeof Toast !== 'undefined' && Toast.show) Toast.show('부양가족 수는 1명 이상이어야 합니다.', 'danger');
                DOMUtils.getElement('#dependents_count').focus(); resultsContainer.style.display = 'none'; return;
            }
            ...
            // Update DOM (ensure NumberUtils is available or use direct formatting)
            const format = (num) => (typeof NumberUtils !== 'undefined' && NumberUtils.addCommas) ? NumberUtils.addCommas(num) : String(num);
            DOMUtils.getElement('#result_annual_salary').textContent = format(annualSalary) + '원';
            DOMUtils.getElement('#result_non_taxable_monthly').textContent = format(nonTaxableMonthly) + '원';
            DOMUtils.getElement('#result_non_taxable_annual').textContent = format(nonTaxableAnnual) + '원';
            DOMUtils.getElement('#result_dependents_count').textContent = dependentsCount + '명';
            DOMUtils.getElement('#result_net_monthly_pay').textContent = format(netMonthlyPay) + '원';
            DOMUtils.getElement('#result_gross_monthly_pay').textContent = format(Math.round(annualSalary / 12)) + '원';
            DOMUtils.getElement('#result_total_monthly_deduction').textContent = format(totalMonthlyDeduction) + '원';
            DOMUtils.getElement('#deduction_national_pension').textContent = format(Math.round(np / 12));
            DOMUtils.getElement('#deduction_health_insurance').textContent = format(Math.round(hi / 12));
            DOMUtils.getElement('#deduction_long_term_care').textContent = format(Math.round(altc / 12));
            DOMUtils.getElement('#deduction_employment_insurance').textContent = format(Math.round(aei / 12));
            DOMUtils.getElement('#deduction_income_tax').textContent = format(Math.round(finalIncomeTax / 12));
            DOMUtils.getElement('#deduction_local_income_tax').textContent = format(Math.round(localIncomeTax / 12));
            resultsContainer.style.display = 'block';

        } catch (error) {
            console.error("연봉 계산 중 오류 발생:", error);
            if (typeof Toast !== 'undefined' && Toast.show) {
                Toast.show("계산 중 오류가 발생했습니다. 입력값을 확인해주세요.", "danger");
            } else {
    ...
    if (calculateButton) {
        DOMUtils.addEvent(calculateButton, 'click', function() {
            ...
            displayResultsUI(annualSalary, nonTaxableMonthly, dependentsCount);
        });
    }
}
